...
public class ServiceTypeForm : Form
{
    ...
    private Panel BuildToolbar()
    {
        ...
        txtSearch = new TextBox
        {
            ...
            BackColor = Color.White, PlaceholderText = "Search services..."
        };
    ...
    private Panel BuildGridCard()
    {
        ...
        lblNoData = UIHelper.CreateEmptyDataLabel("No service types found.");
    ...
    private void RefreshGrid()
    {
        ...
        if (dgv.Columns["Name"]        is { } cNm)  { cNm.HeaderText  = "Service Name"; cNm.FillWeight  = 20; }
        if (dgv.Columns["Category"]    is { } cCat) { cCat.HeaderText = "Category";     cCat.FillWeight = 13; }
        if (dgv.Columns["Price"]       is { } cPr)  { cPr.HeaderText  = "Price";        cPr.FillWeight  = 9; }
        if (dgv.Columns["Description"] is { } cDs)  { cDs.HeaderText  = "Description";  cDs.FillWeight  = 20; }
        if (dgv.Columns["Status"]      is { } cSt)  { cSt.HeaderText  = "Status";       cSt.FillWeight  = 10; }
    ...
    private void BtnAdd_Click(object? s, EventArgs e)
    {
        ...
        VetMS.Forms.Toast.Success("Service saved!");
        LoadData();
    }
    ...
    private void EditRow(int rowIndex)
    {
        ...
        if (dlg.ShowDialog(this) != DialogResult.OK) return;
        item.Name = dlg.Result.Name; item.Category = dlg.Result.Category;
        item.Price = dlg.Result.Price; item.Description = dlg.Result.Description;
        item.IsActive = dlg.Result.IsActive;
        try { DataStore.Update(item); }
        catch (Exception ex) { VetMS.Forms.CustomMessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        VetMS.Forms.Toast.Success("Service updated!");
        LoadData();
    }
    ...
    private void DeleteRow(int rowIndex)
    {
        ...
        VetMS.Forms.Toast.Success("Service deactivated!");
        LoadData();
    }
    ...
    private void RecoverRow(int rowIndex)
    {
        ...
        VetMS.Forms.Toast.Success("Service recovered!");
        LoadData();
    }
...
public class ServiceTypeDialog : Form
{
    ...
    public ServiceTypeDialog(ServiceType? existing = null)
    {
        ...
        {
            Text = isEdit ? "Edit Service Type" : "Add New Service Type",
        ...
        {
            Text = isEdit ? "Update service information" : "Fill in the details below",
        ...
        body.Controls.Add(FieldLabel("Service Name *", lm, y));
        y += 22;
        txtName = StyledTextBox(lm, y);
        body.Controls.Add(txtName);
        y += 42;

        body.Controls.Add(FieldLabel("Category *", lm, y));
        y += 22;
        ...
        body.Controls.Add(FieldLabel("Price", lm, y));
        y += 22;
        ...
        body.Controls.Add(numPrice);
        y += 42;

        body.Controls.Add(FieldLabel("Description", lm, y));
        y += 22;
        ...
        footer.Resize += (_, _) =>
        {
            btnCancel.Left = footer.Width - btnCancel.Width - 20;
            btnCancel.Top = 13;
    ...
    private void BtnSave_Click(object? sender, EventArgs e)
    {
        ...
        {
            VetMS.Forms.CustomMessageBox.Show("Service name is required.");
            return;
        }

        if (cboCategory.SelectedItem == null)
        {
            VetMS.Forms.CustomMessageBox.Show("Category is required.");
            return;
        }
    ...
    private static Button DialogButton(string text, Color back, int width)
    {
        var btn = new Button
        {
            Text = text,
