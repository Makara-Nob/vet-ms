...
public class MedicalRecordForm : Form
{
    ...
    private Panel BuildSearchBar()
    {
        ...
        txtSearch = new TextBox { Left = 28, Top = 13, Width = 300, Font = new Font("Segoe UI", 11f), PlaceholderText = "Search pet, owner, diagnosis..." };
        txtSearch.TextChanged += (_, _) => FilterData();
        var btnAdd = UIHelper.CreateButton("New Record", UIHelper.Success, 100, 31); btnAdd.Left = txtSearch.Right + 12; btnAdd.Top = 12; btnAdd.Click += BtnAdd_Click;
    ...
    private void FilterData()
    {
        ...
        _filtered = string.IsNullOrWhiteSpace(q) ? _data
            : _data.Where(x => (x.PetName?.ToLower().Contains(q) == true) || (x.CustomerName?.ToLower().Contains(q) == true) || (x.Diagnosis?.ToLower().Contains(q) == true) || (x.VetName?.ToLower().Contains(q) == true)).ToList();
    ...
    private void RefreshGrid()
    {
        ...
        var page = _filtered.Skip((_currentPage - 1) * _pageSize).Take(_pageSize)
            .Select(x => new { x.Id, Date = x.CreatedAt.ToString("yyyy-MM-dd"), x.PetName, Owner = x.CustomerName, Vet = x.VetName, Diagnosis = x.Diagnosis.Length > 60 ? x.Diagnosis[..60] + "..." : x.Diagnosis, FollowUp = x.FollowUpDate?.ToString("yyyy-MM-dd") ?? "-" }).ToList();
        dgv.DataSource = page;
        ...
        if (dgv.Columns["Diagnosis"] is { } c5) { c5.HeaderText = "Diagnosis"; c5.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill; }
        if (dgv.Columns["FollowUp"] is { } c6) { c6.HeaderText = "Follow-up"; c6.Width = 100; }
    ...
    private void ViewRow(int row)
    {
        ...
        using var dlg = new MedicalRecordDialog(item, true);
        dlg.ShowDialog(this);
    }
...
public class MedicalRecordDialog : Form
{
    ...
    {
        _readOnly = readOnly;
        Text = existing is null ? "New Clinical Note" : (_readOnly ? "Clinical Note Information" : "Edit Clinical Note");
        ...
        _vets = DataStore.GetUsers()
            .Where(u => u.IsActive && (u.Role == "Veterinarian" || u.Role == "Administrator"))
            .ToList();
        _meds = DataStore.GetMedications().Where(m => m.IsActive).ToList();

        // Usable content width (padding 40 each side + scrollbar 20)
        int cw = formWidth - 100;

        // ── HEADER ──────────────────────────────────────────────────────────────
        var hdr = new Panel { Dock = DockStyle.Top, Height = 110, BackColor = UIHelper.LightBg };
        ...
        hdr.Controls.Add(new Panel { Dock = DockStyle.Bottom, Height = 1, BackColor = Color.FromArgb(220, 220, 225) });
        ...
        var flow = new FlowLayoutPanel
        {
            ...
            WrapContents = false,
            Padding = new Padding(40, 18, 40, 20),
            AutoScroll = true
        };
        ...
        lblApptSelected = new Label
        {
            ...
            ForeColor = Color.FromArgb(30, 100, 50)
        };
        ...
        dgvMeds = new DataGridView
        {
            Width = cw,
            Height = 150,
            AllowUserToAddRows = !_readOnly,
            AllowUserToDeleteRows = !_readOnly,
        ...
        rowFU.Controls.AddRange(new Control[] { chkFollowUp, dtpFollowUp });
        flow.Controls.Add(rowFU);
        flow.Controls.Add(new Panel { Width = cw, Height = 10 });

        // ── BUTTON BAR ───────────────────────────────────────────────────────────
        var pnlBtn = new Panel { Dock = DockStyle.Bottom, Height = 60, BackColor = Color.White };
        var btnSave = UIHelper.CreateButton(_readOnly ? "Done" : "Save Record", UIHelper.Success, 130);
        var btnCancel = UIHelper.CreateButton("Cancel", Color.FromArgb(108, 117, 125), 90);
        btnSave.Top = btnCancel.Top = 13;
        btnSave.Left = pnlBtn.Width - 238;
        btnCancel.Left = pnlBtn.Width - 106;
        ...
        // Dock order matters: Bottom → Fill → Top
        Controls.Add(pnlBtn);
        Controls.Add(flow);
        Controls.Add(hdr);
        ...
        {
            // Pre-select appointment in grid
            ...
            if (existing.FollowUpDate.HasValue)
            {
                chkFollowUp.Checked = true;
                dtpFollowUp.Value = existing.FollowUpDate.Value;
            }
            ...
            if (_readOnly)
            {
                ...
                dgvMeds.ReadOnly = true;
                dgvMeds.Enabled = false;
            }
        }
    }
    ...
    private void RefreshApptGrid()
    {
        ...
        foreach (var a in src)
        {
            ...
            row.Cells["ColStatus"].Style.ForeColor = a.Status == "Completed"
                ? Color.FromArgb(30, 130, 76)
                : Color.FromArgb(180, 100, 0);
            row.Cells["ColStatus"].Style.Font = new Font("Segoe UI", 9f, FontStyle.Bold);

            // Keep pre-selected row highlighted after search
            if (_selectedAppointment?.Id == a.Id)
                row.Selected = true;
        }
    }
    ...
    private void Save(object? s, EventArgs e)
    {
        ...
        {
            VetMS.Forms.CustomMessageBox.Show("Please select a linked appointment from the list.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        if (string.IsNullOrWhiteSpace(txtDiagnosis.Text))
        {
            VetMS.Forms.CustomMessageBox.Show("Diagnosis is required.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            txtDiagnosis.Focus();
            return;
        }
